#!/usr/bin/env python3
"""For a single year, find every (pixel, date) in Sep-Nov where sfcWindmax
exceeds a threshold within the cropping mask (see build_landuse_mask.py),
and record date, year, coordinate, max wind speed, wind direction (from the
daily-mean uas/vas - an approximation, not tied to the specific hour of the
day's peak gust), and GRDC cropping region. Writes one CSV per year; combine
them with combine_exceedances.py.
...
def main(year, start_month, end_month, threshold, mask_path, zones_gpkg, out_dir):
    ...
    speed = prepare_spatial(open_variable("sfcWindmax", year, start_month, year, end_month))
    ...
    zone_id_grid, id_to_name = load_zone_labels(zones_gpkg, speed.isel(time=0), "AEZ")

    lat_vals = speed["lat"].values
    lon_vals = speed["lon"].values
    time_vals = speed["time"].values
    speed_vals = speed.values
    u_vals = u.values
    v_vals = v.values

    exceed = mask[None, :, :] & (speed_vals > threshold)
    ...
    df = pd.DataFrame(
        {
            ...
            "lat": lat_vals[r_idx],
            "lon": lon_vals[c_idx],
            "max_wind_speed": speed_vals[t_idx, r_idx, c_idx],
